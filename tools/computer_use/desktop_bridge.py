...
_CALLER_PRINCIPAL: ContextVar[Optional[Tuple[str, str]]] = ContextVar(
    "desktop_bridge_authenticated_principal",
    default=None,
)


def set_desktop_bridge_caller(principal: Optional[Tuple[str, str]]) -> Token:
    """Bind the verified WS principal for an agent build/turn context."""
    normalized = None
    if principal is not None:
        provider, subject = principal
        normalized = (str(provider).strip(), str(subject).strip())
    return _CALLER_PRINCIPAL.set(normalized)
...
def current_desktop_bridge_scope() -> DesktopBridgeScope:
    ...
    try:
        ...
        profile = ""
    return DesktopBridgeScope(provider, subject, profile).validate()
...
def desktop_bridge_connections_present() -> bool:
    """True when any connector exists; never use this to select a socket."""
    return _BROKER.has_connections()
