...
def test_tool_backend_lookup_dispatches_only_to_current_principal_and_falls_back(
    ...
    """Exercise the real tool resolver/dispatcher against two live scopes."""
    ...
    async def scenario():
        ...
        # Alice and Bob are both connected globally, but an unmatched verified
        # principal must retain the host's own CUA fallback rather than borrow
        # either Desktop socket.
        ...
        # Stopping Alice's connection leaves Bob usable.
        computer_use_tool.reset_backend_for_tests()
...
def test_tui_branch_turn_keeps_parent_profile_bridge_scope(monkeypatch, tmp_path):
    ...
    db.create_session(
        ...
        source="desktop",
        model="test/model",
        authenticated_owner=("ticket", "alice"),
    )
    db.set_session_title(parent_key, "parent")
    db.close()
    ...
    class _Agent:
        ...
        def __init__(self, owned_db):
            self.owned_db = owned_db
    ...
    monkeypatch.setattr(
        server,
        "_make_agent",
        lambda _sid, _key, session_db=None, **_kwargs: _Agent(session_db),
    )
    ...
    async def scenario():
        ...
        assert child["profile_home"] == str(profile_home.resolve())
...
def test_tools_configure_remote_profile_keeps_config_db_and_bridge_scope(
    monkeypatch, tmp_path
):
    """App-global tools changes and reset stay inside the live session profile."""
    import yaml
    ...
    profile_home = root / "profiles" / "work"
    profile_home.mkdir(parents=True)
    launch_config = root / "config.yaml"
    ...
    launch_config.write_text(launch_text, encoding="utf-8")
    ...
    principal = ("stub", "alice")

    class _PublicTransport:
        authenticated_principal = principal
        authorized_profile = "work"
    ...
    def fake_make_agent(_sid, _key, session_db=None, **_kwargs):
        ...
        captured["status"] = desktop_bridge.desktop_bridge_computer_use_status(
            timeout=2
        )
        return types.SimpleNamespace(
            model="test/model", reasoning_config=None, enabled_toolsets=["terminal"]
        )
    ...
    monkeypatch.setattr(server, "_config_model_target", lambda: ("test/model", "test"))
    ...
    server._sessions[sid] = {
        "agent": types.SimpleNamespace(model="old/model", reasoning_config=None),
        "authenticated_principal": principal,
        "history": [{"role": "user", "content": "before reset"}],
        "history_lock": threading.Lock(),
        "history_version": 0,
        "profile": "work",
        "profile_home": str(profile_home),
        "session_key": "20260713_130000_remote",
        "source": "desktop",
        "transport": transport,
    }
    ...
    async def scenario():
        ...
        assert captured["status"]["ready"] is True
        assert default_ws.sent.empty()
    ...
    try:
        asyncio.run(scenario())
        assert launch_config.read_text(encoding="utf-8") == launch_text
        saved = yaml.safe_load(profile_config.read_text(encoding="utf-8"))
        assert saved["marker"] == "work"
