...
def test_delegate_context_snapshot_cleans_reused_worker_after_exception(
    ...
    try:
        with ThreadPoolExecutor(max_workers=1) as executor:
            ...
            # Context.run must also restore the caller thread after an exception.
            assert _current_delegate_scope()["home"] == str(remote_home)
            assert current_desktop_bridge_scope().principal == "desktop-alice"

            # Submit without propagation onto the same recycled worker. Neither
            # the remote profile nor the poisoned caller may remain installed.
