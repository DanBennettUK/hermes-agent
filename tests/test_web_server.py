...
def test_computer_use_status_ignores_bridge_connected_for_another_principal(
    ...
    # Retain a foreign connection in the process to pin the regression: global
    # presence is diagnostic only and must never influence this request's route.
    monkeypatch.setattr(
        "tools.computer_use.desktop_bridge.desktop_bridge_connections_present",
        lambda: True,
    )
    monkeypatch.setattr(
